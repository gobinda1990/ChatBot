...
//			+ " WHERE A.HRMS_CODE = ? ";
	...
	String HRMS_QRY=" SELECT  A.HRMS_CODE, A.CHARGE_CD,E.CHARGE_NM, B.DESIGNATION,  C.MOBILE, "
			+ " CASE  WHEN D.USR_CATG_NM = 'Approver' THEN 'Administrator' "
			+ " ELSE D.USR_CATG_NM END AS USR_CATG_NM  "
			+ " FROM USR_CD A  "
			+ " LEFT JOIN DESIGNATION_CD B ON A.DESIG = B.DESIG_CD	"
			+ " LEFT JOIN IMPACT_USER_PROFILE C ON A.USR_CD = C.USR_CD "
			+ " LEFT JOIN CHARGE_CD E ON A.CHARGE_CD = E.CHARGE_CD	"
			+ " LEFT JOIN USR_CATEGORY D ON A.USR_CATG_CD = D.USR_CATG_CD	"
			+ " WHERE A.HRMS_CODE = ?";
